...
-export([start/1, analize_data/1, analize_file/1, aggregate/1, aggregate/2, reducer/2, worker/2]).
...
wait_data() ->
  receive
    {worker, Pid, Fille, Data} ->
      io:format("reducer got result from ~p, ~p~n", [Pid, Fille]),
      Data
  after
    1000 ->
