...
-export([parse/1, parse_file/2, aggregate/2]).
...
parse_file(File, ReducerPid) ->
    io:format("worker started, ~p~n", [ReducerPid]),
    ...
    Data = lists:foldl(
      fun
        (<<>>, Acc) -> Acc;
        (Line, Acc) ->
        [_Id, Name, Quantity, _Price] = binary:split(Line, <<",">>,[global]),
  ...
  io:format("worker send data, ~p   ~p~n", [ReducerPid, Data]),
  ReducerPid ! {ok, Data},
  ok.
